...
supported release is `v0.1.6`; earlier releases are unsupported.
...
CI and stable-tag validation install `cargo-audit` 0.22.2 with `--locked` and
run `cargo audit` against `Cargo.lock` under read-only workflow permissions.
The same pinned audit also runs every Monday at 06:17 UTC in the isolated
dependency-audit workflow and can be dispatched manually.
