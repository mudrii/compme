...
check_all_self_test_env_contracts() {
  ...
  check_self_test_env_file "$atspi_session_script" \
    COMPME_ATSPI_FIXTURE_SRC COMPME_ATSPI_PROBE_SRC COMPME_ATSPI_KEYTAP_SRC \
    COMPME_ATSPI_APP_NAME \
    COMPME_ATSPI_FIELD_NAME COMPME_ATSPI_WAIT_TRIES COMPME_ATSPI_WAIT_SLEEP \
    COMPME_ATSPI_KEEP COMPME_ATSPI_LIBEXEC
}
...
require_test_symbol "$repo_root/crates/app/src/run_loop_tests.rs" 'grammar_detection_blocks_without_fresh_browser_domain_when_domain_rules_exist' "run_loop grammar domain test"
require_test_symbol "$repo_root/crates/app/src/run_loop_tests.rs" 'grammar_detection_refresh_drops_stale_allowed_browser_domain' "run_loop grammar domain refresh test"
require_test_symbol "$repo_root/crates/app/src/run_loop_tests.rs" 'grammar_detection_respects_enable_per_app_snooze_and_axset' "run_loop grammar gate test"
...
require_line "$development_doc" '^## Linux Host Gate[[:space:]]*$' "DEVELOPMENT Linux host gate section"
require_line "$development_doc" '^cargo check --locked -p platform_macos --target aarch64-apple-darwin --all-targets[[:space:]]*$' "DEVELOPMENT Linux host gate macOS cross-check"
require_line "$check_runner_script" '^[[:space:]]+--fence\)$' "gate runner selects a documented fence by heading"
