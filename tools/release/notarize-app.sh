...
SH
  ...
  grep -Fq "xcrun notarytool submit --wait --timeout 60m --keychain-profile compme-release" "$log"
  ...
  grep -Fq "xcrun notarytool submit --wait --timeout 60m --key $tmp/AuthKey_PATH.p8 --key-id PATH123456 --issuer 11111111-1111-1111-1111-111111111111" "$log"
  ...
  grep -Fq "xcrun notarytool submit --wait --timeout 60m --apple-id release@example.test --password app-specific-password --team-id TEAM123456" "$log"
...
# 60m default matches the hosted workflow and docs/RELEASING.md: a freshly
# enrolled Apple account's first notarizations can queue for hours, and manual
# recovery runs must get the same budget as CI.
notary_args=(--wait --timeout "${COMPME_NOTARYTOOL_TIMEOUT:-60m}")
build_notary_auth_args
