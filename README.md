...
| `platform_windows` | The product uses the fail-closed adapter constructor; optional `with_uia()` provides focused-field capability, text and selection reads but is not product-wired. Subscriptions, mutation and overlay remain stubs. Real host services include owner-only DACL hardening, console Ctrl-C handling and native `ShellExecuteW` URL opening. |
