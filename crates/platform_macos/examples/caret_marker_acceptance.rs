...
struct EventState {
    ...
    accepted_diagnostics: Option<Result<platform_macos::MacosCaretDiagnostics, PlatformError>>,
...
fn main() {
    ...
    let caret_adapter = Arc::clone(&adapter);
    let caret_requirement = requirement.clone();
    ...
    let caret = match adapter.subscribe_caret(Arc::new(move |field, rect| {
        ...
        if state.accepted_diagnostics.is_none() && (rect.is_some() || looks_like_text_field(&field))
        {
            let diagnostics = caret_adapter.caret_diagnostics(&field);
            if diagnostics_accepts(&diagnostics, &caret_requirement) {
                state.accepted_diagnostics = Some(diagnostics);
            }
        }
        println!(
    ...
    while Instant::now() < deadline {
        ...
        let (accepted_from_callback, last_field) = {
            let state = state.lock().expect("state");
            (state.accepted_diagnostics.clone(), state.last_field.clone())
        };
        if let Some(diagnostics) = accepted_from_callback {
    ...
    let (focus_count, caret_count, last_field, accepted_from_callback, apps) = {
        let state = state.lock().expect("state");
        (
            state.focus,
            state.caret,
            state.last_field.clone(),
            state.accepted_diagnostics.clone(),
            state.apps.clone(),
        )
    };
    let diagnostics = accepted_diagnostics.or(accepted_from_callback).or_else(|| {
        last_field
            .as_ref()
