...
//! structure exists and CI can gate it. The read/insert path (AT-SPI2) and the
//! ghost/correction overlay (an override-redirect X11 window — see
//! `x11_overlay`) are real; the remaining AT-SPI2/X11 surfaces — focus and caret
//! events, and the accept tap — are **not yet built**: each is a fail-closed stub
//! returning [`PlatformError::UnsupportedField`] or a safe empty value, so wiring
//! this adapter in is inert, never a crash, and each method's doc names the Linux
//! API its real implementation will use.
//!
