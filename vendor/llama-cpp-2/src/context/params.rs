...
pub struct LlamaContextParams {
    ...
    /// Kept separate from the raw params until context creation so the raw
    /// callback-data pointer is never present in a movable Send/Sync value.
    pub(crate) abort_flag: Option<Arc<AtomicBool>>,
    /// Optional diagnostic signal set only when llama.cpp polls the callback.
    pub(crate) abort_poll_observer: Option<Arc<AtomicU64>>,
}
...
impl Default for LlamaContextParams {
    fn default() -> Self {
        ...
        Self {
            context_params,
            abort_flag: None,
            abort_poll_observer: None,
        }
    }
}

impl LlamaContextParams {
    /// Install a process-shutdown abort flag for decode work.
    ///
    /// The resulting context owns an `Arc` clone for at least as long as
    /// llama.cpp retains the callback-data pointer, so callers cannot create a
    /// dangling callback through this safe API.
    #[must_use]
    ...
    /// Observe that llama.cpp has polled the configured abort callback.
    ///
    /// This is a diagnostic/test seam; it does not request cancellation. The
    /// observer is retained with the callback state for the context lifetime.
    #[must_use]
    pub fn with_abort_poll_observer(mut self, observer: Arc<AtomicU64>) -> Self {
        self.abort_poll_observer = Some(observer);
        self
    }
}
