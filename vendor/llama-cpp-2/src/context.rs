...
pub(crate) struct AbortCallbackState {
    pub(crate) requested: Arc<AtomicBool>,
    pub(crate) polled: Option<Arc<AtomicU64>>,
}
...
pub struct LlamaContext<'a> {
    ...
    /// Owns callback data until after `llama_free` returns in `Drop`.
    _abort_state: Option<Arc<AbortCallbackState>>,
}
...
impl<'model> LlamaContext<'model> {
    pub(crate) fn new(
        ...
        embeddings_enabled: bool,
        abort_state: Option<Arc<AbortCallbackState>>,
    ) -> Self {
        Self {
