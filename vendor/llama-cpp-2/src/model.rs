...
use crate::context::{AbortCallbackState, LlamaContext};
...
impl LlamaModel {
    ...
    pub fn new_context<'a>(
        ...
        let abort_flag = params.abort_flag;
        let abort_poll_observer = params.abort_poll_observer;
        let context = unsafe {
            llama_cpp_sys_2::llama_new_context_with_model(self.model.as_ptr(), context_params)
        };
        let context = NonNull::new(context).ok_or(LlamaContextLoadError::NullReturn)?;

        unsafe extern "C" fn abort_if_requested(data: *mut std::ffi::c_void) -> bool {
            if data.is_null() {
                return false;
            }
            // SAFETY: `LlamaContext` owns this `Arc` state for the whole period
            // in which llama.cpp retains the pointer.
            let state = unsafe { &*(data.cast::<AbortCallbackState>()) };
            if let Some(polled) = &state.polled {
                polled.fetch_add(1, std::sync::atomic::Ordering::Release);
            }
            state.requested.load(std::sync::atomic::Ordering::Acquire)
        }
        let abort_state = abort_flag.map(|requested| {
            std::sync::Arc::new(AbortCallbackState {
                requested,
                polled: abort_poll_observer,
            })
        });
        if let Some(state) = &abort_state {
            // SAFETY: the callback function matches llama.cpp's ABI; the data
            // pointer targets the Arc allocation retained by LlamaContext.
            unsafe {
                llama_cpp_sys_2::llama_set_abort_callback(
                    context.as_ptr(),
                    Some(abort_if_requested),
                    std::sync::Arc::as_ptr(state).cast_mut().cast(),
                );
            }
        }
