...
| **G20** | Info | ✅ **CLOSED 2026-09-17** — remaining blocks annotated per §21.3 in `6ec61d8` (`lib.rs` 67, `shell_host.rs` 2, `tray.rs` 2, `settings_window.rs` 2; `ax_worker.rs` stays 17 + 2 deliberately listed); every CI lane green incl. the macOS compile (run 35264496197). | Done. |
