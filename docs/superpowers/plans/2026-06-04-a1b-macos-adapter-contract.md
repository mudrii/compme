...
- **[CORR 06-07/06-08] Accept-key binding policy (Cotypist parity):** factory/default evidence points to **Tab → Word** (partial accept), **grave/`~` key-above-Tab → Full**, but shortcuts are configurable and installed user profiles may differ. **Implemented 2026-06-07:** the MVP `accept_tap_decision` is keycode-driven — Tab/keycode 48 → Word, grave/keycode 50 → Full, gated on `action.is_some()`. The old Option+Tab modifier override was removed. Live grave-accept validation on a desktop session is pending. A2/A3 must expose shortcut configuration rather than treating these defaults as immutable.

**Known tradeoff (matches Cotypist):** while a suggestion is visible, a literal backtick/grave keystroke is consumed as full-accept and does not reach the field — same class as Tab being swallowed. Backtick-as-text is common in code/markdown, so a **per-app accept-key disable** (as Cotypist offers for Tab) and/or a configurable full-accept key are A2 follow-ups; until then the consume only fires while a ghost is actually showing.
