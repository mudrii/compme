...
**Date:** 2026-06-09
**Status:** The committed A2 code scope is deterministic/build-verified on
current `main`. Pure features are unit-tested and reviewed; GUI/permission-bound
features retain live LOOK residuals. The formerly open SidebarOnly field
detector, full statistical autocorrect, cross-app previous-input mode, and
thesaurus selection trigger were implemented on 2026-07-16 and are tracked as
live-validation work in `docs/ROADMAP.md`. (status 2026-09-09: the code
closures stand — `compat::AppPolicy`, `assistant_field_evidence`,
`NSSpellChecker` in `platform_macos/src/shell_host.rs`, `PreviousInputs`, and
`COMPME_THESAURUS_SELECTION` are all on HEAD — but their runner gate IDs
`sidebar-only-editor-assistant-look`, `full-autocorrect-prose-code-look`,
`cross-app-previous-inputs-look`, `selection-thesaurus-look`, and
`tray-external-links-look` are all "never recorded" in the
`docs/ACCEPTANCE.md` evidence table, as are `apps-policy-toggle-look` and
`personalization-pane-look` for the settings rows below.)
...
`618013d` on `main`; it shipped in v0.1.5 (2026-07-13) and v0.1.6 (current
release), not the v0.1.4 artifact.
**Live pending status (re-verified 2026-07-16):** see
...
|---|---|---|
| **Multi-candidate + cycle** | ✅ `model_client::complete_n` N-sample (greedy + temp/top_k/top_p/seed); `engine_core` `CompletionReadyMulti`/`Cycle` + candidate list; Down-arrow cycle key; accept inserts shown; AcceptWord collapses to active; public `Engine` behavior now has cycle/wrap/accept tests | Deterministic engine/model coverage done. The rebuilt scripted Carbon gate covers Down-cycle dispatch; physical Down-cycle remains UX confirmation. (status 2026-09-09: ROADMAP records a synthetic Down-cycle pass with a real model and `COMPME_CANDIDATES=3` on 2026-07-07; `multi-candidate-cycle-physical-look` is "never recorded" in ACCEPTANCE.) |
| **Previous-input context** | ✅ `context::build_context_block` (bounded, newline-collapsed, opt-in); `app` `PreviousInputs` (`crates/app/src/inference.rs`) maintains redacted, globally deduplicated, recency-ordered, bounded same-app and cross-app rings, recording the latter only while its live opt-in is enabled; Full-accept recording uses the resolved bundle id (not volatile `pid:N`) | Same-app isolation remains the default. Disabling cross-app context clears global history and stops global collection; `cross-app-previous-inputs-look` pins the remaining two-app product-loop evidence (never recorded as of 2026-09-09). **Clipboard context remains a separate source** via `read_pasteboard_text` + run-loop refresh. |
| **Compatibility tiers** | ✅ classifier for Works/SetupNeeded/MirrorOnly/Partial/SidebarOnly/Unsupported/Unknown; Unsupported fails closed. SidebarOnly reads direct focused-field AX metadata and enables only conservative AI-chat/sidebar markers through `Capabilities::assistant_field`. | Classifier/gating tests are done. `sidebar-only-editor-assistant-look` separately pins real VS Code, Cursor, and Windsurf editor/sidebar fields because they are not rows in the exact 13-row PID matrix. (status 2026-09-09: classification is `assistant_field_evidence` in `platform_macos/src/lib.rs`, which retains the AX source + marker before reducing to `Capabilities::assistant_field`; the gate is never recorded.) |
| **British English normalization** (Cotypist 0.22 Labs) | ✅ pure crate `localize` plus host integration: curated US→UK spelling map keyed only on US-only forms (shared spellings untouched), query-case reapplied via shared `crates/textcase::CasePattern`, default **off** via `COMPME_BRITISH_ENGLISH`; replacement offer reaches the shared AxSet accept path. | ✅ §16 live gate passed 2026-06-10 (`color`→`colour`, docs/ACCEPTANCE.md "A2 Local-Replacement Live Gate": the `colour` ghost was offered, placed, and Esc-dismissed live; its accept leg is inferred from the byte-identical shared `Command::Replace` → AxSet path proven by the emoji/autocorrect accepts, not from a separately logged `colour` accept). Non-atomic SyntheticKeys/Clipboard replacements remain deliberately fail-closed pending compatibility evidence; no backspace-synthesis implementation is promised. |
...
## A2 code closures added 2026-07-16

- SidebarOnly editor-vs-sidebar detection via conservative direct AX metadata
...
- Tray **Visit Website** / **Contact Support** one-shot actions through the
  portable `ShellHost::open_url` seam (ROADMAP counts this as the fifth
  closure); `tray-external-links-look` pins browser handoff.
- (status 2026-09-09) The gate IDs above were registered in the runner on
  2026-07-17 (`c42b369`); every one of them is still "never recorded" in the
  ACCEPTANCE evidence table, and v0.1.6 was tagged with none of the 22 rows
  closed.

## Remaining A2 — GUI / permission / live-bound (specified; validation environment-bound)

These rows are implemented to a deterministic/build-verified standard: real
compiling code, pure cores unit-tested, and FFI surfaces build-verified. The scripted live
...
|---|---|---|
| Screen Recording / OCR context | ✅ `platform_macos::screen_recording_permission`/`request_screen_recording_permission` (CGPreflight/Request) + **`screen_context_text`: capture the display containing the caret (fallback main display) → local Vision OCR (`VNRecognizeTextRequest`)**, redacted + bounded, published into a field-tagged `WorkerContext.screen` cell; `COMPME_SCREEN_CONTEXT` opt-in, off by default, degrades to field-only when ungranted. **OCR runs on a dedicated `screen_ocr::ScreenOcr` worker thread** with a bounded latest-slot queue (new requests overwrite stale pending work) so the ~200–800 ms Vision pass never stalls the AppKit run loop / overlay / Carbon accept callbacks (§11 latency floor), and inference rejects stale OCR output when its field no longer matches the completion request. | live OCR quality/perf tuning on a granted desktop, plus multi-display caret-display confirmation. (status 2026-09-09: ROADMAP records a live submit-path pass 2026-07-07 after the CGImageRef encoding panic fix `e5c055b`; not a 22-gate row.) |
| Google Docs / Arc setup onboarding | ✅ `compat::needs_accessibility_setup` (browser/Arc/Dia + unreadable field; tested) wired on the read-context error path — surfaces setup guidance once per app (the Google-Docs-in-Chrome case). Browser-domain extraction has landed separately. | live `setup-needed-docs-arc-onboarding` manual gate in Arc/Docs (never recorded as of 2026-09-09). |
| Browser mirror-window fallback | ✅ `Engine::set_mirror_mode` — MirrorOnly apps (Firefox/Zen) render the ghost in the floating non-activating mirror window (front-app popup anchor) instead of inline; run loop sets it per focused app's tier; engine test pins it. | live Firefox/Zen confirmation. (status 2026-09-09: ROADMAP records the Zen mirror pipeline log-proven end-to-end under organic hardware typing, `f6fa98b` 2026-07-07; `mirror-window-firefox-zen-look` itself is never recorded in ACCEPTANCE.) |
| Terminal/iTerm AI-agent activation | ✅ `compat::terminal_prompt_activates` (sigil-aware; tested) gates terminals to natural-language prompts before submit. | live tuning vs real agent prompts. (status 2026-09-09: ROADMAP records live gating proven 2026-07-07 — command line blocked, natural-language prompt allowed; not a 22-gate row.) |
...
| Compatibility matrix gating | ✅ classifier, Unsupported fail-closed behavior, onboarding, browser-domain allow/exclude smoke paths, and conservative SidebarOnly assistant-field gating. A local/manual pass can write the exact 13-row ledger under `tools/acceptance/evidence/a2/`; CI/tag workflows do not consume it. | pass every matrix row through `tools/release/check-a2-matrix-ledger.sh` locally; run the separate `sidebar-only-editor-assistant-look` gate for VS Code/Cursor/Windsurf editor and assistant fields. (status 2026-09-09: the only committed ledger is `tools/acceptance/evidence/a2/v0.1.0-20260708-154651/` — all 13 rows passed for v0.1.0; it is far outside `check-a2-matrix-ledger.sh`'s default 86400 s age window, and no ledger was recorded for v0.1.5/v0.1.6, so a fresh local pass is required before the next release.) |
...
`cargo build --locked --workspace --all-targets` green (status 2026-09-09: the
full gate is `tools/dev/check.sh`; `platform_macos` and `app` need
`--test-threads=1`, the other crates run in parallel). The canonical automated
...
Compme's trailing-word and exact-selection lookup paths now ship on `main`.
Preserve the public-vs-binary provenance distinction when describing why the
selection mode is parity work rather than a public-help requirement.
...
offer coverage in the app run-loop tests. The separate
`COMPME_THESAURUS_SELECTION` path now carries exact selected text, cycles
multiple candidates, and accepts through exact atomic range replacement;
remaining thesaurus work is live LOOK validation.
