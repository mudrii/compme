...
**(status 2026-09-09:** reconciled against HEAD `1da83f5`. Every P1 surface below still exists; where later phases widened a signature or a menu, the section carries a dated note rather than a rewrite. Nothing in this spec was re-run live for this pass — the live claims stand on their original dates.)
...
(status 2026-09-09: `derive_status` in `crates/app/src/status.rs` now takes **six** inputs — `(accessibility_trusted, accessibility_subscriptions: AccessibilitySubscriptions, secure, model_available, ready, enabled)` — and `BlockReason` gained `AccessibilityUnavailable` and `RelaunchRequired` (between `Permission` and `SecureInput`) and `ModelUnavailable` (between `SecureInput` and `Loading`). The P1 priority order above is preserved as a sub-sequence. Added helpers: `needs_accessibility()` (drives the settings-item visibility) and `render_title`/`render_line` (snooze overlay over `Ready` only). 17 `#[test]`s in the module.)
...
(status 2026-09-09: the flags live in `shell_flags::TrayFlags` as `enabled`, `quit`, `open_settings` plus later A2/A3 flags (`snooze_requested`, `global_disable`/`app_disable`, `open_settings_window`, `check_updates`, …). Since `b8d3626` (G10) every toggle site — tray menu, ToggleGlobal shortcut, `SIGUSR1` — goes through `TrayFlags::toggle_enabled` (one `fetch_xor`), closing the load-then-store race. The run loop persists a user toggle as `COMPME_ENABLED` in `config.env` (`config::persist_setting`), so `enabled` now starts from the persisted value, true on first run.)
...
- **Location:** `$HOME/Library/Application Support/compme/config.env`, overridable with `COMPME_CONFIG=<path>`. (status 2026-09-09: `config::config_file_path_from` resolves per OS — XDG config home on Linux, the per-OS home on Windows — same file name.)
...
- **New keys** (previously hardcoded constants, now config with the same defaults + validation):
  ...
  - (status 2026-09-09: all six defaults/clamps above match `run_loop.rs` `DEFAULT_*` + `Config::from_lookup` verbatim. Later phases added more keys to the same lookup — e.g. `COMPME_CANDIDATES` (default 1, clamp 1..=5) for multi-candidate cycling, `COMPME_ENABLED`, `COMPME_TRAILING_SPACE`, `COMPME_CLIPBOARD_CONTEXT`, `COMPME_SCREEN_CONTEXT` — documented in their own specs, not here.)
...
2. The tray exposes **Open Accessibility Settings** when blocked; the action sets `open_settings_requested`, and the run loop runs `open "x-apple.systempreferences:com.apple.preference.security?Privacy_Accessibility"` (via `std::process::Command`, no AppKit). (status 2026-09-09: same URL, now behind the `ShellHost::open_permission_settings` seam in `crates/platform_macos/src/shell_host.rs`, so the run loop is host-agnostic.)
3. Secure input **and trust** are re-polled on the same ~500ms throttle (every ~40 heartbeats; status 2026-09-09: implemented as a monotonic-time throttle, `SECURE_POLL_INTERVAL_MS = 480`, not a heartbeat count) and fed into the status derivation. Re-polling trust matters: if the user grants Accessibility while the app is running (via the prompt or the tray's settings affordance), `Blocked(Permission)` clears without a restart. (Per-field secure state continues to be handled by the engine via `Capabilities` on focus.)
...
accept keys still work without that permission. (status 2026-09-09: that gate is
still "pending / conditional" in the ACCEPTANCE ledger — never recorded.)
...
(status 2026-09-09: shipped as `crates/platform_macos/src/tray.rs`. `set_status` takes the pre-rendered pieces — `(title, status_line, enabled, needs_accessibility)` — and errors off the main thread instead of no-op'ing. The button carries a template menu-bar icon and falls back to the `menu_title` text only when the icon fails to load (`tray_button_title`); the menu grew a stats line, Disable-in-app/Disable-globally submenus, Snooze, Settings…, and Check-for-Updates/Website/Support links. The action→flag mapping is now the pure `apply_tray_action` and **is** unit-tested (`every_tray_action_changes_only_its_public_flag`). The run loop reaches the tray through the `TrayHandle` seam (`crates/app/src/shell/macos.rs`), not `MacosTray` directly.)

### 6. Retina diagnostic (item 6)

- Expose `MacosPlatformAdapter::display_scales() -> Vec<(ScreenRect-ish bounds, f64 scale)>` (wrap existing `active_display_scales`). (status 2026-09-09: shipped as the free fn `platform_macos::display_scales() -> Vec<(ScreenRect, f64)>`, reached from the run loop as `shell.display_scales()`; the projection `display_scale_pairs` is unit-testable.)
...
`Engine::on_dismiss(&mut self) -> Result<Vec<CompletionRequest>, PlatformError>` wrapping the existing `core::Event::Dismiss` so the run loop can hide a showing ghost the instant the user disables the app. **Unit-tested in `crates/engine`.** (status 2026-09-09: signature unchanged; tests `dismiss_hides_a_showing_ghost`, `dismiss_stales_an_in_flight_request_at_the_engine_layer`, `dismiss_with_nothing_showing_is_noop`. A sibling `on_dismiss_suppress` (Esc dismiss + suppress, A1b Task 5b) was added later.)
...
- Each heartbeat: poll secure input (throttled) → `derive_status(trusted, secure, ready, enabled)` → `tray.set_status(...)`; submit requests only when `status.suggestions_allowed()`; when `enabled` transitions true→false (`should_dismiss_on_disable(prev, now)`), call `engine.on_dismiss()`; if `quit_requested`, break; if `open_settings_requested`, run the settings `open` once and clear it; optional coord diagnostic.
...
- **Visual menu-bar click** of the tray cannot be automated: macOS exposes 0 menu bars for an accessory-policy status item to System Events. The toggle *behavior* is verified via SIGUSR1 + unit tests (`should_dismiss_on_disable`, `derive_status`, `suggestions_allowed`); only the literal mouse-click on the menu item is manual.
...
Per-app settings/personalization, per-domain browser controls, multi-candidate UI, local encrypted memory, optional Screen Recording / OCR context, native inline-prediction suppression, IME composition, sentence/punctuation stop-boundary, Windows/Linux adapters. Automated multi-monitor geometry correction beyond the existing pixel/point guard (revisit only if measurement shows an offset).

(status 2026-09-09: several of these have since shipped under A2/A3 and are tracked in `docs/ROADMAP.md`, not here — per-app/per-domain steering (ROADMAP 2.1 ✅); multi-candidate cycling (`Engine::on_completion_multi`/`on_cycle`, `COMPME_CANDIDATES`; the physical `multi-candidate-cycle-physical-look` gate is still never recorded); encrypted local memory (`crates/memory`: redact → AES-256-GCM → SQLite, opt-in `StorageMode`, Keychain key; the `encrypted-memory-all-monitored-live` gate is partial). Redaction runs before encryption via `crates/redaction`; since `b8d3626` (G13) a lone `/` is no longer an entropy signal in `looks_high_entropy` — digits, mixed case, and `+`/`=` still trip, so 32+-char lowercase URLs/paths survive. Screen-context OCR is behind `COMPME_SCREEN_CONTEXT` (`crates/app/src/screen_ocr.rs`). `platform_linux` is wired for AT-SPI2/X11 with documented unsupported surfaces; `platform_windows` remains the fail-closed scaffold. IME composition, sentence stop-boundary, and native inline-prediction suppression were not re-verified in this pass.)

**[CORR 06-08] Removed from this out-of-scope list: KV-cache reuse and the long-lived model actor — both are now implemented and closed (design spec §15 G3); see `model_client::LlamaModel` persistent context + `reusable_prefix_len`.**
