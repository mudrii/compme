...
| **D14** | **Model download UX under-specified.** Cotypist's public troubleshooting (`/help/troubleshooting`) adds requirements not captured: **direct-download recovery** when the CDN download fails, **manual model placement** (drop a GGUF into the models dir), and **hardware gating** for large Gemma-4-class models (don't offer models the machine can't run). | Medium (A3 backlog) | **Added to A3 model catalog/download acceptance** (§9): the download flow must handle failure recovery, allow manual GGUF placement, and gate large models by available RAM/hardware. **[2026-06-10]** model_catalog (4 entries, license gates, RAM advisory) + model_fetch pure core (streaming sha256, resume planning) shipped 2026-06-10; download loop + picker UI pending. **[2026-06-12]** Download loop SHIPPED (c122: Setup-tab “Download Recommended Model” → ModelDownloader worker); catalog expected_sha256 threaded to verify-before-rename (c126 — engages once a hash is pinned; all entries currently None); model-license click-through gate at the download edge SHIPPED (c127 — per-model NSAlert, fail-closed, persisted via COMPME_LICENSE_ACCEPTED). Picker UI still pending. |
