...
> **Implementation status (2026-07-17):** this is the historical design record,
> not the current pending-work ledger. The macOS reference product is
> shipped through signed/notarized v0.1.5, and the committed macOS parity code
> gaps are closed on current `main`; 22 runner-pinned macOS manual/live gates
> plus additional manually recorded Tier-4 rows remain open. Windows/Linux
> remain fail-closed scaffolds.
> (status 2026-09-09: the current release is signed/notarized **v0.1.6**
> (`workspace.package.version`, tag `v0.1.6`); `docs/ACCEPTANCE.md` records
> that none of the 22 gate rows were closed when it was tagged. `platform_windows`
> is still the fail-closed scaffold; `platform_linux` is a wired AT-SPI2 +
> passive-X11 adapter with explicit unsupported surfaces — see `docs/ROADMAP.md`.)
...
**2026-06-07 architecture pivot (Tauri → native):** The app shell was planned as a Tauri v2 tray app, but the shipped implementation is a **native Rust binary** (`crates/app`) with an AppKit `NSStatusItem` tray built via `objc2` (`crates/platform_macos/src/tray.rs`) — **there is no Tauri dependency in the workspace**. Config is a dotenv-style `config.env` file (not a hidden Tauri webview). Mentions of "Tauri v2 tray app", `apps/app`, `WebviewWindowBuilder`, `run_on_main_thread`, and the Tauri `updater` plugin below are **superseded**: read them as the native AppKit run loop, `crates/app`, the objc2 tray/menu, and an A3 updater decision still open (no Tauri) — the updater decision has since been made (GitHub-release manifest + release-page handoff, §1 "Update" row / `docs/RELEASING.md`). The cross-platform `PlatformAdapter` seam is unchanged.
...
- **AX worker run loop** — owns AX observer resources and transient Carbon accept-hotkey resources; callbacks answer from **pre-computed state** and never perform synchronous AX reads.
  (status 2026-09-09: still accurate, with two details the design never named. The worker lives in `crates/platform_macos/src/ax_worker.rs`; every observer binding also starts a **focused-element safety poll** at `CARET_SAFETY_POLL_INTERVAL` = 250 ms (4 Hz) as a backstop for missed AX notifications, and caret deliveries are de-duplicated by a `CaretCoalescer` with `CARET_COALESCE_INTERVAL_MS` = 25 (`lib.rs`), newest event always delivered. Carbon registration is dispatched onto this worker through `AxWorkerHandle::install_resource` (`accept_tap_installer` → `install_worker_accept_tap_resource`): the per-suggestion **accept** keys (hotkey ids 1–4, `WorkerAcceptTapResource`) are transient, while the always-on shortcuts (ids 5–9: force-activate, toggle-app, toggle-global, grammar check/accept, `WorkerShortcutResource`) are registered once per accept subscription for the process lifetime.)
...
crates/shell_flags      # product-shell state vocabulary shared by app + platform_macos (settings/tray flags, key-chord grammar) — added 2026-07-22
...
crates/grammar          # grammar/capitalization fixes distinct from typo autocorrect (2026-07 grammar-fix design)
...
crates/platform_macos   # AX read, Carbon accept hotkeys, NSPanel overlay, front-app, NSStatusItem tray (objc2); AX worker split into ax_worker.rs, tests in lib_tests.rs
crates/platform_windows # fail-closed Windows adapter scaffold behind the PlatformAdapter contract
crates/platform_linux   # (status 2026-09-09) wired AT-SPI2 read/write/events + passive X11 accept tap + X11 overlay; explicit unsupported surfaces (see docs/ROADMAP.md)
...
(status 2026-09-09: the shipped `platform::PlatformAdapter` keeps this shape but every method returns `Result<_, PlatformError>`, `caret_rect` returns `Result<Option<ScreenRect>>`, and the trait gained `subscribe_accept`, `insert_replacing`, and `insert_replacing_range`. `Capabilities` adds `assistant_field` and `security_state`; `InsertStrategy` adds `NativeRangeSet` and `ImeCommit` for the Windows/Linux adapters. `platform::ux_mode` exists as designed.)
...
7. Overlay renders the active candidate at `caret_rect` (Retina/multi-monitor coordinate conversion, §12) or popup fallback. Current shipped UI is a transparent non-activating panel with one label; Down cycles candidates through `UpdateGhost`. Backdrop styling and visible multi-candidate list rows/badges remain future display/UI scope. Avoid double ghost text where the target integration allows native inline prediction suppression; for cross-app Accessibility fields this is deferred out of A1b.
8. **Accept**: full-completion shortcut inserts all; partial shortcut inserts next word (+ trailing space if available). **[CORR 06-07] Match Cotypist defaults: Tab = accept next word (partial), `~`/key-above-Tab = accept full** (`acceptNextWordOnly_includeTrailingSpaceIfAvailable=1`, live-confirmed); Tab disengageable per-app where Tab has native function. Down cycles candidates; Esc dismisses and suppresses completions in the current field until refocus/edit; Option+Tab passes a literal Tab because only bare Tab is registered as a Carbon hotkey. **[CORR 06-12 — D11 fixed in code and scripted]** Deterministic unit coverage exists for Esc, Option+Tab, and cycle semantics, and the rebuilt NSApp-pumping harnesses make synthetic Tab/grave/Esc/Down dispatch through `RegisterEventHotKey`. Physical-key runs remain UX confirmation; per-app/global enable toggles stay A3 (§8).
...
- **Tag synthetic events** (`CGEventSource.userData`) and skip them in the tap — else your own insert re-enters the tap → dismiss/double-insert. (status 2026-09-09: the tag is written — `tag_synthetic_event` sets `SYNTHETIC_EVENT_TAG` on every self-posted key/paste event — but it is **not consulted** on the production path: `is_self_generated_event` is `allow(dead_code)` and the Carbon hotkey handler hard-codes `source_user_data: 0`. Re-entrancy is prevented instead by arming the accept hotkeys only while a suggestion is visible. Qfd G18; `docs/ARCHITECTURE.md` "Accept Interception" carries the same correction.)
...
- Backend: llama.cpp via `llama-cpp-2` (latest v0.1.146; **`metal` is NOT default** — must enable; add `sampler`). Vendored build via git submodule + cmake; needs clang/cmake; pin exact versions. (status 2026-09-09: `=0.1.146` is exact-pinned per target in `crates/model_client/Cargo.toml` — `metal` on macOS, `default-features = false` elsewhere — and resolved through `[patch.crates-io]` to the vendored copy at `vendor/llama-cpp-2` (same release plus the abort-callback lifetime extension), in both the root workspace and `tools/spike`.) `mistral.rs` (pure Rust, Metal) is the plan-B if the C++ build hurts distribution.
...
- **Optional local memory (deferred within A2)** (status 2026-09-15: shipped as `crates/memory::MemoryStore` — AES-256-GCM (`aes-gcm`) records over `rusqlite` with `StorageMode::{AcceptedOnly, AllMonitored}`, `delete_all`/`delete_app`; retrieval is recency-based (`recent(app, limit)`), **no FTS5 index and no ranker similarity score** — the ranker never sees stored records. Retrieval is now WIRED: `run_loop::hydrate_previous_inputs` replays an app's stored records into the previous-input prompt ring on its first focus, behind the same enable gate as the write path (both resolve it through `previous_input_context_chars`). Between 2026-09-09 and 2026-09-15 `recent` had no production caller at all, so the store was write-only and this status line overstated what shipped. Erase-all is wired too (Apps-pane button → `delete_all`, which also clears the live rings). **Two parts of this bullet remain unmet:** per-domain deletion needs a schema column and is blocked behind the 0.x schema policy below; and "disable and erase" only works in that order, because `StorageMode::Off` drops the store handle and with it the deletion UI (see ARCHITECTURE's `memory` section for why re-opening it needs a load-without-create key API first). The key comes from `platform_macos::keychain`, host toggles `COMPME_MEMORY*`): encrypted `rusqlite`/SQLite-compatible store with FTS5-style retrieval of accepted completions for retrieval-augmented prompting + ranker similarity score. Opt-in (`TrainingDataCollector` — encrypted, local, record count + "disable and erase"), inspectable. Encryption key must live in Keychain or an equivalent macOS-protected key store; deletion UX must support all data plus per-app/per-domain data. Plain unencrypted `rusqlite` is not Cotypist parity.
...
- Optional Screen Recording / screen-aware context must be local-only, off-revocable, and non-blocking: the app continues with field-only context when denied. (status 2026-09-09: shipped — `COMPME_SCREEN_CONTEXT` gates an off-main-thread Vision OCR worker in `crates/app/src/screen_ocr.rs`; redacted result consumed only when the request stamp still matches.)
...
**`COMPME_*` config keys for the pure-feature toggles** are now read by `app`: `COMPME_EMOJI*`, `COMPME_AUTOCORRECT`, `COMPME_FULL_AUTOCORRECT`, `COMPME_THESAURUS`, `COMPME_THESAURUS_SELECTION`, `COMPME_BRITISH_ENGLISH`, and `COMPME_ACCEPT_*_KEY` (plus, since the 2026-07 grammar-fix design, `COMPME_GRAMMAR_FIX` and `COMPME_GRAMMAR_{CHECK,ACCEPT}_KEY`; the always-on shortcut keys are `COMPME_FORCE_ACTIVATE_KEY` / `COMPME_TOGGLE_APP_KEY` / `COMPME_TOGGLE_GLOBAL_KEY`). The original list was `COMPME_EMOJI` (enable) + `COMPME_EMOJI_SKIN_TONE` + `COMPME_EMOJI_GENDER` (`crates/emoji`), `COMPME_THESAURUS` (trailing-word auto mode), `COMPME_AUTOCORRECT` (curated typo-fix enable), and `COMPME_ACCEPT_WORD_KEY` + `COMPME_ACCEPT_FULL_KEY` (keycodes, `platform_macos::AcceptKeymap`). Current `main` adds the separate full-statistical-autocorrect and exact-selection thesaurus controls; their remaining work is the runner-pinned live LOOK, not host wiring.
...
| **A3 settings + ship** | 2–3 | Native settings UI (all §8 panes; no Tauri) + per-app/domain overrides + **model catalog/download (incl. download-failure recovery, manual model placement, and hardware gating for large models — §15 D14)** + diagnostics + pause/snooze + **launch-at-login (`SMAppService`/login-item, default-off, toggleable — §15 D13)** + GitHub-release-driven update manifest + codesign/notarize (hardened runtime + entitlements) + **OSS license (Apache-2.0, `LICENSE` added 2026-06-09) + no-proprietary-telemetry (local-only/opt-in) + model-license passthrough** | Installable signed/notarized `.app` zip; configurable; self-diagnosing; Accessibility onboarding plus optional Screen Recording onboarding; update manifest validated; **no pricing/feature gates (all features open — Project Scope); Apache-2.0 LICENSE present; no network analytics by default; model downloads surface their license** |

~9–13 weeks solo to a shippable macOS app.

(status 2026-09-09: A3 deliverables are shipped on `main` — the 9-tab settings `NSWindow` (`platform_macos/src/settings_window.rs`), launch-at-login via `SMAppService` (`login_item.rs`, `COMPME_LAUNCH_AT_LOGIN`), custom GGUF override (`COMPME_MODEL_PATH`), catalog download with license click-through and RAM gating (§15 D14), snooze/per-app exclude, and the notarized GitHub-release + Homebrew cask path through v0.1.6. The residual is the live/LOOK evidence pinned in `docs/ACCEPTANCE.md`.)
...
- **Accept-key interception (current design)** = **Carbon `RegisterEventHotKey`** registered only while a suggestion is armed; Tab accepts next word, grave accepts full, Esc dismisses+suppress, Down cycles. **[CORR 06-09 — §15 F1]** This avoids Input Monitoring and matches Cotypist's no-CGEventTap architecture. (status 2026-09-09: "only while armed" holds for the four accept ids; the always-on shortcut chords — force-activate, toggle-app, toggle-global, grammar check/accept — are registered for the process lifetime by `install_process_shortcut_hotkeys`, and bindings carry Carbon modifier masks via `AcceptKeymap` (`from_accept_keys_with_mods_and_grammar`).)
...
**Feature status:** emoji completion, curated typo-fix and full statistical
autocorrect, spelling localization, trailing-word and exact-selection thesaurus,
opt-in deduplicated cross-app previous inputs, and signed/unsigned reversible
web-driven config (mandatory confirmation for both) are shipped on current
`main`. Their remaining environment-bound validation is pinned in
`docs/ACCEPTANCE.md`. Domain/website overrides are no longer optional for
personalization/privacy parity; they are A2/A3 requirements for browser use.
...
| **F2** | **Insertion default order is AxSet-first**; Cotypist's primary path is **pasteboard paste** (smart-insert + backspace + pasteboard restore), char/chunk fallback. We have all strategies but a different default. | **Closed by project decision:** keep the shipped fixed `AxSet → SyntheticKeys → Clipboard → None` order. Revisit only if the live compatibility pass proves an in-scope app needs a different strategy. (status 2026-09-09: that order is a **capability-probe preference**, not a runtime cascade — `insertion_strategy` in `platform_macos/src/lib.rs` picks `AxSet` when the AX value is settable, else `SyntheticKeys` when global insert is allowed and the selected range is settable, else `Clipboard` when a caret rect exists, else `None`; `insert_impl` then runs exactly the chosen strategy. The only runtime fallback is `finish_axset_insert`: an `AxSetApply::SilentlyIgnored` write retries **once** via `SyntheticKeys` for plain inserts (live-validated in iTerm2, 2026-06-10), while replacements (`replace_left > 0`) fail closed with `CannotComplete`; `SyntheticKeys`/`Clipboard` never fall through to another strategy.)
  *(status 2026-09-10, plan item 2d / Qfd G2: before the `SilentlyIgnored` classification stands, both insert paths re-poll the readback a bounded three reads / 20 ms apart — an asynchronously applied `AXValue` write now classifies `Applied` — and the `SyntheticKeys` retry is gated on a bundle allowlist seeded with iTerm2 (the only live-validated app), so unlisted apps fail closed with `CannotComplete` rather than risk a double insert. Chromium-family behaviour to be recorded from the caret-marker live gates when they next run.)* |
...
| **D14** | **Model download UX under-specified.** Cotypist's public troubleshooting (`/help/troubleshooting`) adds requirements not captured: **direct-download recovery** when the CDN download fails, **manual model placement** (drop a GGUF into the models dir), and **hardware gating** for large Gemma-4-class models (don't offer models the machine can't run). | Medium (A3 backlog) | **Added to A3 model catalog/download acceptance** (§9): the download flow must handle failure recovery, allow manual GGUF placement, and gate large models by available RAM/hardware. **[2026-06-10]** model_catalog (4 entries, license gates, RAM labels) + model_fetch pure core (streaming sha256, resume planning) shipped 2026-06-10. **[2026-06-12]** Download loop SHIPPED (c122: Setup-tab “Download Recommended Model” → ModelDownloader worker); catalog expected_sha256 threaded to verify-before-rename (c126); model-license click-through gate at the download edge SHIPPED (c127 — per-model NSAlert, fail-closed, persisted via COMPME_LICENSE_ACCEPTED). **[2026-06-15]** all built-in catalog entries have pinned SHA-256 values, and the Setup model picker/download row exists. **[2026-06-17]** `Exceeds` RAM verdict is enforced as a hard pre-license/pre-fetch block; `Fits`/`Tight` remain offerable labels. Residual: live LOOK/recovery validation and any broader multi-model picker polish. |
...
|---|---|---|
| Tab→Word, key-above-Tab→Full | A1b | Deterministic mapping + rebuilt scripted Carbon gates close repeatable dispatch evidence (§15 G6/I11); physical-key runs are supplemental UX confirmation and Input Monitoring revoked remains a manual permission-state spot-check. |
...
| Per-app Tab disable + per-app/domain overrides | A2/A3 | Override store gates suggestion/accept per app + per domain; round-trip test + live two-app check. (status 2026-09-09: shipped in `crates/prefs` + `app` — `COMPME_TAB_DISABLED_APPS`, `COMPME_{ENABLED,DISABLED,EXCLUDED}_APPS`, `COMPME_EXCLUDED_DOMAINS`, per-feature `*_ON_APPS/_OFF_APPS`; Tab suppression flows to `TAB_HOTKEY_SUPPRESSED` in the Carbon arm; Chrome domain-exclude live-proven 2026-06-17; `apps-policy-toggle-look` LOOK gate never recorded.) |
| Accept-key reconfiguration | A2/A3 | User can rebind both accept keys; persisted; takes effect without restart. **Shipped**: `platform_macos::AcceptKeymap` (pub) — keycode→binding map, `from_accept_keys(word, full)` rebinds the two accept keys with collision + negative-keycode validation; `accept_tap_decision`, Carbon registration (`carbon_bindings`), AND the handler's id→keycode inverse (`keycode_for_hotkey_id`) are all data-driven from one keymap (default preserves exact Cotypist bindings). The app reads `COMPME_ACCEPT_WORD_KEY`/`COMPME_ACCEPT_FULL_KEY`, the Shortcuts recorder persists live rebinds, and the tap is rearmed without restart. Residual: force-activate/global-toggle shortcut surfaces and broader LOOK validation. (status 2026-09-09: the force-activate / toggle-app / toggle-global shortcuts are shipped as always-on Carbon ids 5–7 with `COMPME_FORCE_ACTIVATE_KEY` / `COMPME_TOGGLE_APP_KEY` / `COMPME_TOGGLE_GLOBAL_KEY`; remaining residual is the `always-on-hotkeys-physical-look` and `shortcuts-recorder-look` gates, both "never recorded" in `docs/ACCEPTANCE.md`.) |
...
(status 2026-09-09: all four sources are wired in `app` behind default-off keys — `COMPME_CLIPBOARD_CONTEXT`, `COMPME_PREVIOUS_INPUT_CONTEXT`, `COMPME_CROSS_APP_PREVIOUS_INPUTS` (redacted, deduplicated five-entry ring), `COMPME_SCREEN_CONTEXT` (`screen_ocr.rs`). Screen-context submit path passed live 2026-07-07; `cross-app-previous-inputs-look` is never recorded.)
...
(status 2026-09-09 — what has actually been live-verified, per `docs/ACCEPTANCE.md` and `docs/ROADMAP.md`: the tier classifier is `crates/compat` (`CompatTier::{Works, SetupNeeded, MirrorOnly, Partial, SidebarOnly, Unsupported}`, `terminal_prompt_activates`). **Live evidence exists for** TextEdit (accept/replacement/snooze/memory runs), Safari address bar and a Chrome `<textarea>` (accept, 2026-06-10), iTerm2 (AxSet-readback → `SyntheticKeys` fallback, 2026-06-10), Terminal.app/iTerm AI-prompt gating (2026-07-07: command line blocked, natural language allowed), and Chrome domain-exclude (2026-06-17). **Not live-verified:** Mail/Word/Notes/Notion/Obsidian/Messages, Google Docs/Arc/Dia (`setup-needed-docs-arc-onboarding`), Firefox/Zen (`mirror-window-firefox-zen-look`), Slack, VS Code/Cursor/Windsurf (`sidebar-only-editor-assistant-look`), and the Chromium/Electron caret-marker calibration gates — all "never recorded". The 2026-06-09 integration-phase design claims only the TextEdit and iTerm2 runs above.)
...
| Feature | Phase | Acceptance gate |
|---|---|---|
| Emoji completion (skin-tone/gender prefs) | A2/A3 | Emoji suggested from text; preference honored; toggleable. **Suggester implemented**: `crates/emoji::suggest` detects a trailing `:shortcode` (start/whitespace-anchored, ≥2-char prefix or exact, alias table) and renders the glyph honoring `EmojiPrefs` skin-tone (Fitzpatrick, orthogonal to gender for the neutral variant) + gender (neutral/female/male ZWJ); returns `replace_chars` (typed length) for the host and is covered by deterministic unit tests. **WIRED (cycle 26):** run_loop offers the emoji ghost on a typed `:shortcode` (`replacement_offer`/`replacement_decision`), accept emits `Command::Replace` → AxSet range-replace; `COMPME_EMOJI` (+`_SKIN_TONE`/`_GENDER`) enable toggle, default off; gated + race-free (AxSet-only). **LIVE §16 GATE PASSED (2026-06-10):** physical-key Tab accept in TextEdit deleted the typed `:smile` and inserted `😄` on the caret line (ACCEPTANCE.md, A2 Local-Replacement Live Gate). A plain-insert AxSet readback fallback was live-validated in iTerm2, but non-AxSet replacements remain fail-closed residual work because SyntheticKeys/Clipboard cannot atomically delete `replace_left` and insert the replacement. Accept paths also verified in Safari's address bar and a Chrome textarea; Chromium/iTerm2 caret rects normalized for placement. |
| Thesaurus (auto + selection mode) | A2/A3 | Synonym suggestion on word selection / auto mode; toggleable. **Implemented:** `crates/thesaurus::synonyms(word)` returns curated, case-preserving alternatives. `COMPME_THESAURUS` drives trailing-word offers through the local replacement path; `COMPME_THESAURUS_SELECTION` carries exact selected text separately, presents multiple candidates, preserves the cycled candidate across duplicate AX notifications, and accepts through an exact atomic scalar range. Residual: runner-pinned `selection-thesaurus-look` live UX validation. |
| Full autocorrect vs typo/suggested-fix | A2/A3 | Typo fix distinct from full autocorrect (separate toggles, per `/help`); no false-correct in code fields. **Implemented:** `COMPME_AUTOCORRECT` drives the high-precision curated table; `COMPME_FULL_AUTOCORRECT` drives macOS `NSSpellChecker` only for whole single-word results on known prose or positively identified assistant fields. The Apps AC policy gates both, while browsers, unknown apps, editor main panes, and code-like contexts fail closed. The curated TextEdit gate passed 2026-06-10; the OS-backed residual is pinned by `full-autocorrect-prose-code-look`. |
...
| Web-driven config (`setPreference`/`setOverride` deep links) | A3 | URL-scheme deep link applies a per-app/domain override; signed/validated; user-visible. **Parser + application implemented**: `crates/webconfig::parse_deep_link` strictly validates `compme://setOverride?...` (scheme/command/param allow-list, app XOR domain, strict `true`/`false`, charset+length-bounded scope, fail-closed on anything unknown, with deterministic unit coverage); `prefs::apply_override` maps the reversible command onto the policy store (App enable = full allow that also clears exclude). Restricted to a **reversible, user-visible** subset deliberately. **Signing implemented**: `parse_deep_link_with_trust` verifies a trailing `&sig=` Ed25519 signature (128 hex, `verify_strict`, byte-prefix payload — no canonicalization, sig must be final param) against a host-pinned `TrustedKey`; no key configured → signed links rejected fail-closed and every trust branch is unit-tested. Any future non-reversible command must be gated on `LinkTrust::Signed`. **Reception SHIPPED + validated live 2026-06-10**: the bundle declares the scheme (CFBundleURLTypes, c80) and `platform_macos::install_url_event_handler` (NSAppleEventManager kAEGetURL) feeds the run loop, which parses fail-closed, applies the override, persists the round-trippable web-config override keys, fires the dismiss edge, and logs every outcome — a scripted `open compme://…` round-trip applied an Exclude (persisted), rejected a garbage command, and restored via Include. **Signed links validated live 2026-06-10**: a link signed by the `sign_link` dev example verified against `COMPME_TRUSTED_KEY` and applied (`(Signed link)` logged); a tampered payload was rejected (`signature verification failed`). **Confirmation prompt SHIPPED + blocking-verified live 2026-06-10**: every link routes through the pure `prompt_decision_for_link` and a blocking NSAlert (Cancel is the default button; declined = rejected, prefs untouched — test-pinned); scripted runs proved the modal HOLDS the link until answered and an Allow click applied+persisted. Residual: the trusted-key distribution decision (ship-time choice, not code) and a polished Allow/Cancel LOOK pass. |
...
(status 2026-09-09 for the rows above without their own status text: multi-candidate cycle is shipped — `COMPME_CANDIDATES`, Down = Carbon id 4, synthetic Down-cycle live-proven 2026-07-07, `multi-candidate-cycle-physical-look` never recorded; pause/snooze is shipped and live-verified 2026-06-10 with `COMPME_EXCLUDED_APPS` per-app exclusion; configurable length is `COMPME_MAX_WORDS` (clamped 1..=50, default `DEFAULT_MAX_WORDS` = 8); mid-line is `COMPME_MIDLINE` (+ per-app on/off lists); custom model override is `COMPME_MODEL_PATH` behind `LocalModel`; per-app display overrides (font/backdrop/smart-quotes/text-mirroring/size-thresholds), Labs flags, and the quality/reuse thresholds are **not implemented**.)
