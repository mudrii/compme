...
| Personalization | **Prompt-based**: `userPrompt` custom instructions + strength control + sender name/email + optional training-data collector. Static binary evidence shows Gentle/Balanced/Strong symbols; current public pricing describes tier-capped slider stops. | Same mechanism: prompt-based primary; A2 must re-verify factory/current strength semantics and model tier caps explicitly |
...
crates/platform_macos   # AX read, CGEventTap, NSPanel overlay, front-app, NSStatusItem tray (objc2)
crates/engine           # wires SuggestionMachine → adapter + overlay + accept subscription
crates/app              # [CORR 06-07] native complete-me binary: run loop, inference thread, config.env, tray
...
2. **Debounce** ~120–300 ms of keystroke quiescence (**P1 ships a 120 ms default**, configurable via `COMPLETE_ME_DEBOUNCE_MS`, clamp 0..=5000); gate: not mid-word unless configured, not on backspace, min context length.
...
8. **Accept**: full-completion shortcut inserts all; partial shortcut inserts next word (+ trailing space if available). **[CORR 06-07] Match Cotypist defaults: Tab = accept next word (partial), `~`/key-above-Tab = accept full** (`acceptNextWordOnly_includeTrailingSpaceIfAvailable=1`, live-confirmed); Tab disengageable per-app where Tab has native function. Shift-equivalent cycles candidates; Esc dismisses. **[CORR 06-09 — D11] Control semantics still unimplemented (Cotypist parity gap):** Cotypist's **Esc** both dismisses *and* **suppresses completion in the current field** until refocus/edit, and **Option+Tab passes a literal Tab** (per-app bypass). Our `engine_core` has `Dismiss` but **no Esc keyboard tap path, no per-field suppressed state, and no Option+Tab passthrough** in `accept_tap_decision` — **scheduled as A1b Task 5b** (Esc→dismiss+per-field suppress; Option+Tab→literal Tab passthrough; §15 D11). Per-app/global enable toggles stay A3 (§8).
...
- **Strength control [CORR 06-08/06-09 — resolved to 6-stop, `/pricing` authoritative]**: `cotypist.app/pricing` is the source of record — the slider has **6 stops**, only endpoints labeled (**Off** ↔ **Max**), with **tier caps: Free→tick 2, Plus→tick 4 (default), Pro→tick 6/Max**. (Note: the `/help/personalization` page still uses simplified "Off to Strong" wording — do not treat it as confirming six-stop; §15 D2.) The static binary symbols `featurePersonalization{Gentle,Balanced,Strong}` + `Pro.PersonalizationBeyondBalanced` / `Plus.PersonalizationBeyondGentle` are tier-gating *thresholds*, not the stop count — they reconcile with the 6-stop slider. A2 target: **6-stop slider with tier-gated reach** (or a documented divergence if we ship no pricing tiers). Controls how hard instructions + memory steer.
...
- Onboard **Accessibility + Input Monitoring** for the P0/P1 accept path; add optional Screen Recording onboarding in A2+ for screen-aware context. Re-check after grant (may need relaunch). **[CORR 06-07; superseded by §15 D1/F1]** This describes *our current* CGEventTap design: our Tab-swallow tap needs Input Monitoring. **Cotypist itself uses no CGEventTap at all** — it swallows accept keys via Carbon `RegisterEventHotKey` (which can consume) and needs only Accessibility. The Input-Monitoring requirement is therefore *our* divergence, under decision in A1b Task 5c — prefer dropping the tap for a Carbon hotkey to minimize permission prompts.
...
| Personalization | Global custom instructions · per-app and per-domain custom instructions · strength control with current tier caps re-verified · sender name/email · training-data collection (enable / disable+erase / record count) · encrypted local database status |
...
| Permissions | Accessibility status · Input Monitoring status · optional Screen Recording status · pasteboard permission |
...
Tier semantics: Cotypist currently gates completion quotas, model catalog size, clipboard awareness, global/per-app instructions, full autocorrect, Labs, and device count by tier. Complete Me does not need to copy pricing, but A3 must make an explicit product decision: either model feature gates in prefs/entitlements or document a deliberate no-pricing-gates divergence. Do not leave tiered behavior implicit if claiming Cotypist parity.
...
| **A3 settings + ship** | 2–3 | Native settings UI (all §8 panes; no Tauri) + per-app/domain overrides + **model catalog/download (incl. download-failure recovery, manual model placement, and hardware gating for large models — §15 D14)** + diagnostics + pause/snooze + **launch-at-login (`SMAppService`/login-item, default-off, toggleable — §15 D13)** + updater details (native, Sparkle-leading/TBD — no Tauri) + codesign/notarize (hardened runtime + entitlements) + telemetry decision + tier/feature-gate decision | Installable signed/notarized `.app`; configurable; self-diagnosing; Accessibility/Input Monitoring onboarding plus optional Screen Recording onboarding; updater artifacts and telemetry policy validated; pricing/tier divergence either implemented or documented |
...
| Risk | Sev | Mitigation |
|---|---|---|
| **Tab/accept interception** in *our* design uses CGEventTap + **Input Monitoring**. **[CORR 06-08 — §15 D1/F1]** A passive `NSEvent` global monitor cannot swallow keys, but a **Carbon `RegisterEventHotKey` can** — that is how Cotypist swallows Tab with no tap and Accessibility-only. So "global-shortcut cannot swallow" is true only for NSEvent monitors, not Carbon hotkeys. | High | A0 proves the tap path; A1b Task 5c decides tap-vs-Carbon; surface missing-permission state; gate consumption by app-focus/context |
...
- **Accept-key interception (our design)** = **CGEventTap** (`.cgSessionEventTap`, `.defaultTap`, return nil to swallow), needs **Input Monitoring**. **[CORR 06-08 — §15 D1/F1]** The earlier blanket claim "Carbon hotkeys / `NSEvent` global monitors cannot consume keys" was wrong: `NSEvent` global monitors are passive, **but Carbon `RegisterEventHotKey` registers a real hotkey that DOES consume** — Cotypist uses exactly that to swallow Tab without any CGEventTap and with Accessibility only. A CGEventTap is one way to consume, not the only way; the Carbon path avoids the Input-Monitoring prompt. A1b Task 5c decides which we ship.
...
- **Cotypist 2026.1 website/app delta**: optional Screen Recording improves context through local screen text recognition; clipboard context is optional/off by default; personalization is opt-in and encrypted locally; anonymous crash/usage telemetry exists and is user-controllable. Complete Me must model each of those as explicit A2/A3 decisions, not inferred behavior.
...
**What we adopt:** prompt-based personalization (global+per-app/per-domain, **6-stop strength slider Off↔Max, tier-capped 2/4/6** — §15 D2), configurable shortcut matrix, word-capped length, pasteboard + previous-input context, optional screen-aware context, selectable model catalog (base+instruct), backdrop/mirror overlay, disable-native-inline-prediction where possible, pause/snooze, per-app overrides (incl. tab-key/smart-quotes/size-threshold/display), encrypted local stats/training data, compatibility guidance, quality/reuse thresholds.
**What we change:** **[CORR 06-07]** native Rust shell (`crates/app` + objc2/AppKit tray), **not Tauri**; updater + codesign/notarize is an open A3 decision (Tauri dropped — **Sparkle is the leading candidate, as Cotypist ships it**); Rust instead of Swift; `engine_core`/`model_client` built by hand (Cotypist's completion is Swift `CompletionManagerActor` + llama.cpp; `RepliesSDK` is unrelated feedback tooling). **[CORR 06-08]** Our input layer currently uses a consuming **CGEventTap** (→ Input Monitoring), whereas Cotypist ships **no CGEventTap** (AX + CGEvent synthesis + Carbon hotkeys) — revisit per §15 F1/D1; model fetch from HF or self-host TBD; telemetry disabled unless explicitly designed later.
**Deferred features:** emoji completion, thesaurus, full autocorrect, cross-app previous inputs, web-driven config, subscription/seats. Domain/website overrides are no longer optional for personalization/privacy parity; they are A2/A3 requirements for browser use.
...
|---|---|---|
| **F1** | **Accept-key interception requires Input Monitoring.** Our consuming `CGEventTap` (`platform_macos` `_observer_tap`/`consumer_tap`, `lib.rs:1744`) returns `PermissionMissing{"Input Monitoring"}`. **[CORR 06-08 — confidence raised to HIGH, see D1]** Full re-decompile of the bundle (main + every framework) found **zero `CGEventTapCreate`/`CGEventTapEnable`** — Cotypist ships **no event tap at all**. Its input layer is AX + CGEvent synthesis + Carbon hotkeys; the Tab swallow is most likely a Carbon `RegisterEventHotKey` Tab registration. So Cotypist needs only Accessibility, and **we impose an Input-Monitoring TCC prompt Cotypist avoids entirely.** Corroborating: Cotypist's public privacy/permission docs list **Accessibility = required**, Screen Recording = optional, Clipboard = optional, with **no Input-Monitoring onboarding** mentioned. **Requirements are not "met" until this extra prompt is removed or deliberately accepted as a divergence.** | **Scheduled → A1b Task 5c (decided go/no-go).** Spike a Carbon `RegisterEventHotKey` Tab/full-accept and measure cross-app consume; **if it consumes**, drop the tap + Input Monitoring (Accessibility-only parity); **if not**, keep the tap and accept Input Monitoring as a *documented, onboarded* divergence. Result recorded in `tools/spike/FINDINGS.md`; this row closes when the branch is taken. No longer an open-ended question. |
...
Live run on Apple M4 Max, macOS 25.5, Accessibility + Input Monitoring granted. Initial gates ran single-display; G7 was later re-run with two displays (built-in 2× Retina + external 1×).

| ID | Gate | Status |
|---|---|---|
| **G6** | live grave→Full desktop accept | **CLOSED (2026-06-08).** `tools/acceptance/e2e-complete-me.sh` full-accept run against TextEdit: binary logged `accept Full` and the document received the completion via a grave (key code 50) keystroke intercepted by the consuming tap. |
| **I11** | P0 E2E under current bindings | **CLOSED (2026-06-08).** Both modes PASS: full (grave→`accept Full`) and word (`COMPLETE_ME_E2E_ACCEPT=word`: Tab→`accept Word`, grave→`accept Full` remainder). Real-`LlamaModel` live run also exercised the full focus→read→infer→ghost→grave-accept→insert path with genuine inference. |
...
**Remaining highest-leverage actions (now all scheduled, not open):** F1 → **A1b Task 5c** (Carbon-consume go/no-go); D11 control semantics → **A1b Task 5b**; D12 → b74 re-decompile task; G7 live-2× re-confirm (hardware-bound). G7's only residual is the latent true-backing-scale detection caveat above (revisit only if a pixel-reporting app surfaces).
...
|---|---|---|---|
| **D1** | **Cotypist ships no CGEventTap.** Re-decompile found zero `CGEventTapCreate` in the bundle; input = AX + CGEvent synthesis + Carbon hotkeys. Our consuming tap forces Input Monitoring; Cotypist avoids it. | High | Folded into **F1** (raised to high confidence). Prototype Carbon-hotkey accept to drop Input Monitoring. |
| **D2** | **Personalization slider is 6-stop, not 3 — but only `/pricing` is authoritative for it.** `cotypist.app/pricing` shows **6 ticks** (Off↔Max), tier caps Free=2 / Plus=4 / Pro=6. **[CORR 06-09]** The `/help/personalization` page uses **older/simplified "It runs from Off to Strong" wording** (not six-stop), so do **not** claim both pages confirm six-stop — `/pricing` is the source of record. Gentle/Balanced/Strong (binary flags) are tier *thresholds*, not stops. | Medium (A2) | **Resolved in §6 + §1 table:** target a 6-stop tier-capped slider, citing `/pricing` as authoritative. Doc fixed. |
...
| **D8** | **P0 grave→Full live-gate self-contradiction:** §15 G6/I11 say CLOSED (2026-06-08) while `p0-mvp-integration-design.md`, `ACCEPTANCE.md`, `a1b` still call it pending. | High (trust) | **Reconciled:** §15 G6/I11 (M4 Max live run, `e2e-complete-me.sh`) is authoritative; per-phase docs updated to reference it. |
...
| ID | Finding | Severity | Resolution / action |
|---|---|---|---|
| **D11** | **Cotypist control behavior is under-modeled.** Public docs (`/help/tips`, `/help/shortcuts`): **Esc dismisses + suppresses the current field**, **Option+Tab sends a literal Tab** (per-app Tab bypass), temporary per-app toggle, global toggle. Our code has `Dismiss` (`engine_core/src/lib.rs:315`) but **no keyboard path for Escape, no current-field suppression state, and no Option+Tab bypass semantics** (`platform/src/lib.rs:114`). | Medium | **Scheduled → A1b Task 5b.** Esc→dismiss + per-field `suppressed` flag (cleared on refocus/edit); Option+Tab → `None` in `accept_tap_decision` (literal Tab passthrough). Acceptance + unit tests specified in Task 5b. Per-app/global toggle stays A3 settings (§8). Requirement carried in §4 step 8. |
