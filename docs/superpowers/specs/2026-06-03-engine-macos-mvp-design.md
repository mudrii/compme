...
**2026-06-09 PROJECT SCOPE (authoritative — supersedes any "tier/feature-gate decision" left open below):** Complete Me is an **open-source, multi-platform** re-implementation of **all Cotypist functionality EXCEPT payment, licensing, subscription tiers, and multi-device seats** (the only things deliberately not cloned). Consequences, locked:
...
|---|---|---|
| **F1** | **Accept-key interception requires Input Monitoring.** Our consuming `CGEventTap` (`platform_macos` `_observer_tap`/`consumer_tap`, `lib.rs:1744`) returns `PermissionMissing{"Input Monitoring"}`. **[CORR 06-08 — confidence raised to HIGH, see D1]** Full re-decompile of the bundle (main + every framework) found **zero `CGEventTapCreate`/`CGEventTapEnable`** — Cotypist ships **no event tap at all**. Its input layer is AX + CGEvent synthesis + Carbon hotkeys; the Tab swallow is a Carbon `RegisterEventHotKey` Tab registration. So Cotypist needs only Accessibility, and **we impose an Input-Monitoring TCC prompt Cotypist avoids entirely.** | **DECIDED 2026-06-09 (Task 5c) — GO for Carbon; migration gated on one manual live-consume confirm.** Spike `tools/spike/.../p8_carbon_hotkey.rs` ran (M4 Max): **bare Tab (48) and grave (50) both `RegisterEventHotKey` with status 0 (noErr), no modifiers** — the one flagged uncertainty (can bare Tab register?) is answered **yes**; Carbon needs only Accessibility, no Input Monitoring. Combined with the D1 decompile precedent (Cotypist swallows Tab via this exact mechanism, Accessibility-only), the decision is **adopt Carbon, drop the consuming `CGEventTap` + Input Monitoring**. **Residual:** live cross-app *consume* + Input-Monitoring-revoked confirmation in our binary is a manual GUI step (focus TextEdit, press Tab → `HOTKEY_FIRED` + no tab inserted) that can't run headlessly. **Until that confirm + the production migration land, the live-validated CGEventTap remains the shipping mechanism and Input Monitoring is a documented, onboarded divergence with the exit path identified.** Evidence + decision in `tools/spike/FINDINGS.md` (Task 5c). |
...
|---|---|---|
| Tab→Word, key-above-Tab→Full | A1b | ✅ closed (§15 G6/I11): live grave→Full + Tab→Word on TextEdit |
...
| Accept-key reconfiguration | A2/A3 | User can rebind both accept keys; persisted; takes effect without restart |
...
| Feature | Phase | Acceptance gate |
|---|---|---|
| Emoji completion (skin-tone/gender prefs) | A2/A3 | Emoji suggested from text; preference honored; toggleable |
| Thesaurus (auto + selection mode) | A2/A3 | Synonym suggestion on word selection / auto mode; toggleable |
| Full autocorrect vs typo/suggested-fix | A2/A3 | Typo fix distinct from full autocorrect (separate toggles, per `/help`); no false-correct in code fields |
| Web-driven config (`setPreference`/`setOverride` deep links) | A3 | URL-scheme deep link applies a per-app/domain override; signed/validated; user-visible |
