...
> **Last updated:** 2026-09-20 · **Branch:** `main` · v0.1.6 (tag `v0.1.6`) remains the latest published artifact · **Tests:** ≈2232 workspace tests (macOS inventory enforced by native CI; 44 spike tests separate)
>
...
> baseline/warnings rework; the six Batch 3+4 quality-and-release-gate items
> (corpus model-quality gate at its 20/21 baseline, branch-CI model-backed
> smoke gate, AX insertion-serialization tests, `run()` startup extraction,
> post-publish `post_verify` job, version-docs reconciliation check) planned in
> [`2026-07-18-quality-and-release-gates.md`](superpowers/plans/2026-07-18-quality-and-release-gates.md);
> the nitpick sweep; and the 2026-07-19 full-audit remediation
> ([`2026-07-19-audit-findings-remediation.md`](superpowers/plans/2026-07-19-audit-findings-remediation.md))
> covering the version-docs extension, governance-checker rework,
> quality-corpus pins, and post_verify hardening.
>
> Committed 2026-07-21/22 (`17da332`, `9fcc177`, `4cbf0a8`, `ffba136`): the
> strict stable-tag cask guard; governance-wording and cross-platform-guide
> corrections; monthly dependabot for actions; the `shell_flags` crate
> placement (platform's `shell.rs` down to the two portable traits, 26
> workspace crates); and the first god-file series — `builders.rs` and
> `loop_state.rs` out of `run_loop.rs`, `ax_worker.rs` out of
> `platform_macos/lib.rs`, teardown order preserved.
>
> Committed 2026-07-26 (2026-07-25 audit remediation, `Qfd.md` §14): the
> ARCHITECTURE crate-count fix plus a checker pin for it; llama-cpp-2 excluded
> from both dependabot cargo groups (it is exact-pinned once per target, so a
...
> 2026-09-10 (`e3c8d37`, `74e428e`, `b4ae361`, `7462f33`, and 2d as
> `b26caca` + `c93b508`: the G2 bounded readback re-poll plus the
> iTerm2-seeded bundle allowlist on the synthetic fallback, with the MVP
...
> G6 closed 2026-09-16: the failing log was finally pulled, and both
> failures were test-side (no barrier against the async callback
> dispatcher; the expectation compared the input seed against the
> `ax:ptr=`-formatted output identity) — fixed in `cb013ad`. Auditing the
> same commit then found G21, a real defect in its *other* half: the
...
|---|---|---|
| 1 | Clear buffered monitored text after successful app/global memory erasure | Implemented and Astra-reviewed: successful erasure clears captured pending/partial text and live history; cancelled/failed deletions preserve it. App deletion failures no longer publish success. Regression tests cover subsequent flush/typing, scoped app identity and real SQLite deletion failures; portable gates pass. [Combined native CI](https://github.com/mudrii/compme/actions/runs/35455324901) includes the Windows startup-test correction |
| 2 | Disarm stale X11 accept grabs after a layout change collides with another client's grab | Implemented and Astra-reviewed: mapping failures disarm stale grabs; explicit same-layout rebinds retain rollback. Regression failed before the fix; 41 live Linux tests and 138 unit tests pass; [native CI passes](https://github.com/mudrii/compme/actions/runs/35453485745) on `34c5c93` |
| 3 | Support basename-only `COMPME_CONFIG` paths for startup and persistence | Implemented and Astra-reviewed: basename lock/persistence paths work while empty paths and unsafe parents retain their existing refusal. Both regressions failed before the fix; 50 config unit tests and two Linux startup integration tests pass. Windows CI exposed a test assumption, corrected in `d7be740` to require its documented fail-closed startup boundary; [combined native CI](https://github.com/mudrii/compme/actions/runs/35455324901) |
| 4 | Reconcile current documentation and guard the actual published-version claim | Implemented and Astra-reviewed: checker pins the actual release claim as well as the tag reference. Review hardening binds the expected version directly to the claim, rejecting a stale claim even with a current checkout version on the same line; regression reproduced the false pass before the fix |
...
| 29 | Optional off-mac GPU backends | Linux Vulkan, bounded `sm_75` Linux CUDA 12.9 and Windows Vulkan SDK builds verified; independent backend dispatch and regression policy implemented. Fresh Quadro RTX 4000 Vulkan latency, cancellation and quality gates pass. Windows CUDA coverage and other target runtime acceptance remain open |
| 30 | Per-platform performance baselines | Same-binary paired Linux medians: CPU 1,833 ms (fails), Vulkan 120 ms (passes unchanged 500 ms budget); three samples each on Quadro RTX 4000. Repeatable CPU/Vulkan benchmark runner implemented 2026-09-20: builds once, preserves samples/median/provenance, and rejects skipped or unproven runs; self-tests run in CI/release validation. Runner validation produced CPU 1,765 ms (budget failure) and Vulkan 75 ms (pass) medians using separate artifacts. [Measured evidence](PERFORMANCE-2026-09-19.md) records samples and controls. CPU strategy and other target baselines remain open |
...
suite has since grown: the current tree carries **42 live tests** (39 AT-SPI/X11 adapter tests + 1 each for confirm, keyring, reveal).
