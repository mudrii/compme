...
| 3 | 🟠 **SHELL WRITTEN — UNVALIDATED (2026-06-30)** — Apps-pane editing rows (3.1) | M | Core + AppKit shell both landed (build+clippy+tests green). `editAppPolicy:` checkboxes → `apps_edit` signal → run-loop resolves row→app → `set_app_policy_field` → persist. **LOOK to confirm:** checkboxes render/position correctly; toggling actually changes per-app behavior; initial check state (currently defaults OFF — run loop must publish per-row policy bits to pre-check). |
| 4 | 🟠 **SHELL WRITTEN — UNVALIDATED (2026-06-30)** — Always-on hotkeys (3.4) | M | Core + FFI shell landed. `ShortcutAction` (now in `platform` crate) registered via `registration_plan` + dispatched through the Carbon handler → `TapControl::Shortcut` → `HostEvent::Shortcut`. **LOOK to confirm:** global keys actually fire (incl. during no-suggestion windows; may need process-lifetime install vs per-arm-cycle). ForceActivate now calls `Engine::on_force_show` (`Event::ForceShow` re-presents the held `showing` candidate, no new inference; 3 engine tests). ToggleApp/ToggleGlobal call real mechanisms. **Deferred:** re-show only works while a suggestion is held — retaining a *dismissed* candidate across snapshot-invalidating events is a larger change (TODO(LOOK) in `engine_core`). |
| 5 | 🟠 **SHELL WRITTEN — UNVALIDATED (2026-06-30)** — Personalization pane (3.2) | L | Core (live `set_profile` reload) + pane shell landed. New "Personalization" pane (3 knobs: global_instructions, sender name/email, strength) → `personalization_edit` signal → run loop applies + `set_profile` (live) + `persist_setting` (survives restart). **Roadmap correction:** MemoryStore is governed by `config.memory.mode`, NOT the profile — no profile-driven memory lifecycle exists. **LOOK to confirm:** pane renders (PANE_COUNT 8→9, tab indices shifted); single→multi-line instructions field; edits visibly re-steer output. |
