...
| 3 | 🟢 **PANE BUILDS (headless-validated) — VISUAL LOOK pending (2026-06-30)** — Apps-pane editing rows (3.1) | M | Core + AppKit shell landed (build+clippy+tests green). `editAppPolicy:` checkboxes → `apps_edit` signal → run-loop resolves row→app → `set_app_policy_field` → persist. **Headless LOOK confirmed:** app launches and the Settings window opens with all 9 panes, **0 panics** — pane/checkbox construction is crash-free. **Still needs eyes/fingers (pure visual LOOK):** checkboxes render/position correctly and toggling changes per-app behavior. **Roadmap correction (2026-07-01):** the pre-check concern is already resolved in code — `compose_apps_policy_bits` publishes each app's live `enabled`/`tab_disabled`/`mid_line`/`autocorrect` bits on show, seeded via `refresh_apps_policy_checkbox_states`, pinned by `apps_policy_bits_resolve_per_app_overrides_in_checkbox_order`. |
