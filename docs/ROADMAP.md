...
| 3 | 🟡 **CORE DONE (2026-06-30)** — Apps-pane editing rows (3.1) | M | Logic core shipped: `Prefs::set_app_policy_field` + `AppPolicyField` write back `enabled`/`tab_disabled`/`mid_line`/`autocorrect` per app (tested through the resolution getters). **Remaining (LOOK-gated):** AppKit row checkboxes wired to the setter in `settings_window.rs`. |
| 4 | 🟡 **CORE DONE (2026-06-30)** — Always-on hotkeys (3.4) | M | Logic core shipped: Carbon hotkey ids 5–7 (disjoint from accept-key ids), `ShortcutAction` + `shortcut_action_for_hotkey_id` decode, `ShortcutBindings::registration_plan` (tested). force-activate settled = force-show pending. **Remaining (LOOK-gated):** the FFI `RegisterEventHotKey` loop + Carbon-handler dispatch (`#[allow(dead_code)]` on the decode until wired). |
| 5 | 🟡 **CORE DONE (2026-06-30)** — Personalization pane (3.2) | L | Live-reload refactor shipped: `PersonalizationProfile` now lives in `Arc<Mutex>` shared with the worker (spawn signature unchanged); `InferenceHandle::set_profile` reloads it live, read per-request (tested). **Remaining (LOOK-gated):** the three pane controls calling `set_profile` + `MemoryStore` mode lifecycle. |
