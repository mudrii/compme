...
> **Last updated:** 2026-07-27 · **Branch:** `main` · v0.1.5 (`14ae81e`) remains the latest published artifact · **Tests:** ≈1977 workspace tests listed on the current tree (44 spike tests separate)
>
...
questions about a field it is handed), the overlay (2.5), and the
session-dependent ShellHost services (2.6). Until the event half and the overlay
land, the Linux adapter is a complete read/write/accept seam with no runtime that
drives it, which is why `capabilities` still reports `overlay_at_caret: None`.
