...
|----------|---------|--------------|
| [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) | push to `main` / `spike/**`, PR, or `workflow_dispatch` (`paths-ignore` skips only pushes limited to unpinned prose under `docs/superpowers/plans/**` plus release notes, troubleshooting, `Qfd.md`, and `LICENSE`) | Runs the branch checks detailed below: macOS root/policy gates, the separate spike workspace, native Windows/Linux portability, the private Linux live suite, and workflow lint. |
| [`.github/workflows/docs.yml`](../.github/workflows/docs.yml) | push to `main` / `spike/**` touching `docs/superpowers/plans/**`, `docs/RELEASE-NOTES-*.md`, `docs/TROUBLESHOOTING.md`, `Qfd.md`, `LICENSE`, or the workflow itself, or `workflow_dispatch` | The unpinned-prose counterpart to `ci.yml`'s `paths-ignore`, and the exact mirror of that list: one Ubuntu job running the version-docs and privacy-policy checks, `bash -n` over the acceptance/bundle/release scripts, error-severity shellcheck over `tools/**/*.sh`, and Homebrew cask Ruby syntax (`ruby -c Casks/compme.rb`). Same branch scope as `ci.yml`, because pull requests already get these checks from `ci.yml`'s check job. |
...
| [`.github/workflows/release.yml`](../.github/workflows/release.yml) | protected stable tag `vX.Y.Z` | Preflight validates the stable version, exact default-branch tip, and bundle metadata. Validation installs pinned `cargo-audit` 0.22.2 and runs root/spike gates and release self-tests plus the model-backed gates — the release model-gate wrapper and the corpus-based Model-quality gate (`tools/release/check-quality.sh`) — while separate Windows/Linux jobs run the portable-workspace clippy/test plus app-binary gates (fmt runs once on the macOS lane); the Linux job additionally verifies the emitted ignored-test count, runs the AT-SPI harness self-test, and runs the live AT-SPI adapter suite, so a tag validates the same live Linux surface a push to `main` does. Secretless prebuild produces an exact-arm64 binary. Protected signing downloads and re-verifies it, signs, notarizes, staples, deletes the signing keychain, packages the zip, then expands the final zip and requires exactly one top-level `Compme.app` that passes strict `codesign`, staple, and Gatekeeper assessment, and finally attests build provenance for the packaged zip. Publication verifies the downloaded zip's checksum and provenance attestation, writes the update manifest from that checksum, and creates a draft after an exact-tip check; it then re-fetches tag/default branch immediately before undraft, deleting the stale draft and failing on drift. The cask finalizer re-verifies checksum and attestation, then verifies its local zip against the published checksum asset before branch mutation. A closing `post_verify` job (`needs: finalize_cask`) downloads the published assets, verifies the zip against its checksum asset, installs the published cask with `brew install --cask compme`, assesses the installed app with strict codesign, staple, and Gatekeeper checks, and runs a bounded startup smoke. All jobs have explicit timeouts: the outer signing job allows 360 minutes, while the notary submission defaults to 60 minutes and can be raised with `COMPME_NOTARYTOOL_TIMEOUT` below the outer ceiling. |

### Branch CI checks

- The macOS root lane runs `cargo fmt --all -- --check`, workspace clippy,
  parallel portable tests, serial `platform_macos`/`app` tests, all-target builds,
  macOS examples, strict rustdoc, and the cached-GGUF model smoke through
  `COMPME_REQUIRE_LATENCY_BUDGET=0 bash tools/release/run-model-gates.sh`.
- Its policy and packaging checks cover shell syntax/shellcheck, bundle metadata
  and its self-test, version docs and their self-test, release-version validation,
  `ruby -c Casks/compme.rb`, app/icon assembly self-tests, bundle smoke and its
  self-test, UI-assisted/A1b/E2E self-tests, agent-brief and privacy checks plus
  their self-tests, governance self-test, missing-model self-test and product
  smoke, model-client feature policy plus self-test, release model-gate policy
  and self-test, model-gate/quality/check-runner self-tests, cask update/finalize
  self-tests, and notarization/update-manifest self-tests.
- The separate `tools/spike` lane runs `cargo fmt -- --check`,
  `cargo clippy --locked --all-targets -- -D warnings`, `cargo test --locked`,
  and `cargo build --locked --bins`.
- Windows and Linux clippy/test the portable workspace and build `app`. Linux
  also runs `RUSTDOCFLAGS="-D warnings" cargo doc --no-deps --workspace --exclude platform_macos`,
  pinned `cargo-audit`, `tools/release/check-linux-live-test-count.sh`, the AT-SPI
  harness self-test, and
  `tools/acceptance/run-linux-atspi-session.sh --run-in-session cargo test --locked -p platform_linux -- --ignored --test-threads=1`
  with `COMPME_KEYRING_EXPECT=absent`.
...
the cached, pinned GGUF). The wrapper CPU-forces the full root `model_client`
latency suite with `COMPME_MODEL_GPU_LAYERS=0`, then on Darwin reruns the A32
in-flight cancellation test with production Metal offload (`999`); the separate
spike integration test remains Metal/GPU-oriented. Hosted tag validation runs
...
3. On a model-capable Mac, run the release model-gate wrapper before tagging.
   ...
   `COMPME_REQUIRE_MODEL_TESTS=1`, `COMPME_REQUIRE_MODEL_CONTEXT=1`, and
   `COMPME_REQUIRE_LATENCY_BUDGET=1`; on Darwin it reruns the A32 cancellation
   case with `COMPME_MODEL_GPU_LAYERS=999`, then runs the spike integration test
