...
7. Let the next release prove `post_verify` operationally, including final cask/checksum consistency. — **closed 2026-08-26:** the v0.1.6 run executed `post_verify` and its manual closure is recorded (`2d18c34`; §20).
...
| F10 `post_verify` unproven | TRACK — job content re-verified substantive and gate-pinned; awaiting first real release run. **Closed 2026-08-26 by the v0.1.6 release run (§20).** |
...
| F10 post_verify unproven | **TRACK** — first real exercise comes with the next tag **Closed 2026-08-26 by the v0.1.6 release run (§20).** |
...
3. Let the next release prove `post_verify` operationally. — **closed 2026-08-26:** the v0.1.6 run executed `post_verify` and its manual closure is recorded (`2d18c34`; §20).
...
| Doctests | none exist (`--doc` ran 0 tests then; doctests were added later and CI runs them since `ec3247e`) → the `--all-targets` lanes lose no coverage | new check, no gap |
...
| F10 `post_verify` | TRACK | **Still TRACK.** Job present at `release.yml:698-773`, `needs: finalize_cask`, tag-gated; no release has run it (latest published artifact is still v0.1.5). **Closed 2026-08-26 by the v0.1.6 release run (§20).** |
...
| Secure release pipeline | Strict tag guard, provenance, post-publish verification all in place | Aligned; `post_verify` still unproven operationally (F10) **Closed 2026-08-26 by the v0.1.6 release run (§20).** |
...
Passed: `cargo fmt --all -- --check`; `cargo clippy --locked --workspace --all-targets -- -D warnings`; `cargo test --locked --workspace --all-targets -- --test-threads=1` (**1,936 passed / 0 failed / 6 ignored**); `cargo test --locked --workspace --doc` (0 tests then; doctests exist since `ec3247e` and CI runs them); `tools/release/check-version-docs.sh`; `bash tools/release/check-model-gates.sh`; `tools/release/finalize-cask.sh --self-test`; `tools/release/check-github-governance.sh --repo mudrii/compme` (live, exit 0, six accepted caveats); full-repo relative-link check (0 missing); live GitHub run/PR inventory.
...
6. **Let the next release prove `post_verify` (F10)**, including cask/checksum consistency. — **closed 2026-08-26:** the v0.1.6 run executed `post_verify` and its manual closure is recorded (`2d18c34`; §20).
...
| F3 / F10 / Windows-Linux adapters | **F10 closed 2026-08-26** by the v0.1.6 run (§20); F3 and the adapters still OPEN — owner action and the largest committed deliverable respectively. |
...
1. **F3 / F10 / Windows-Linux adapters** — F10 closed by the v0.1.6 run (2026-08-26, §20); F3 and the adapters unchanged: owner action, largest deliverable.
...
|---|---|---|---|---|---|
| G1 | High | platform_linux / app | Linux product **aborts at startup** when no X11 accept tap is installable (no `DISPLAY`, Wayland, or a WM already grabbing Tab): `subscribe_accept` returns `UnsupportedField`, the Linux `ShellHost` default reports `accessibility_trusted() == true`, so `subscription_error_action` maps it to `Fatal` and `run()` returns `Err`. The `ln1-clean-degradation-without-x` gate that would catch it is unchecked. Focus/caret already use `AccessibilityUnavailable` for the same condition. | `crates/platform_linux/src/lib.rs:330-332`, `crates/app/src/run_loop.rs:3437-3445,3505-3521`, `crates/platform/src/shell.rs:29-31`, `docs/MANUAL-VALIDATION-LINUX.md:88` | CONFIRMED, **FIXED** `b8d3626` |
| G2 | High | platform_macos | AxSet readback is taken immediately after the set; an app that applies `AXValue` writes asynchronously reads back the original, is classified `SilentlyIgnored`, and the adapter re-posts the same text as synthetic keys — a **double insert**. Only iTerm2 evidence backs the fallback today. | `crates/platform_macos/src/lib.rs:4459-4477,1360-1386` | PLAUSIBLE, **FIXED** `b26caca` (+`c93b508` gate ordering, 2026-09-10, plan item 2d): both insert paths re-poll the readback through `axset_readback_with_re_poll` (three reads, 20 ms apart, fail-open on read errors) before the `SilentlyIgnored` classification stands, and `finish_axset_insert` gates the synthetic retry on a bundle allowlist seeded with iTerm2 (injectable `BundleIdForPidProvider`; staleness and secure input still refuse first — `c93b508`). Unlisted apps fail closed with `CannotComplete`. MVP §15 F2, the integration spec, and ARCHITECTURE were amended in the same commit. Five tests, mac-lane green (run 34439157981, platform_macos 357). Chromium-family behaviour still to be recorded from the caret-marker live gates |
| G3 | High | platform_macos | `read_required_ax_string_attribute` converts through `CFString::to_string()`, which stops at the first unconvertible UTF-16 unit (lone surrogate, reachable from JS-backed fields). `insert_for_field` then rewrites the **whole value** from the truncated string; the pre-write snapshot check compares two identically truncated reads and cannot catch it. | `crates/platform_macos/src/lib.rs:4741-4744,4436-4457` | PLAUSIBLE, **FIXED** `e3c8d37` (2026-09-10, plan item 2a): the read now copies the exact UTF-16 units (`CFStringGetCharacters`) and refuses via strict `String::from_utf16` plus the `char_len`/`encode_utf16().count()` round-trip predicate with `UnsupportedField { "AX value not round-trippable" }`; `AxRangeTarget::read_value` forwards here, so both insert paths are covered. Unit-pinned on a lone-surrogate CFString. Note: with the pinned core-foundation 0.10.1 the old `to_string()` path did not silently truncate — its `Display` conversion asserts full convertibility and **panics** on exactly that input, so the live failure mode was a contained worker panic. mac CI lane green (run 34432782232, 2026-09-10; lone-surrogate unit test passes) |
...
| G14 | Low | webconfig | Signed `compme://` deep links carry no nonce or expiry, so a captured link verifies forever. Mitigated: only reversible enable/exclude commands exist and every link prompts. Add `&exp=` inside the signed prefix before any non-reversible command lands. | `crates/webconfig/src/lib.rs` (re-locate by symbol) | CONFIRMED, **FIXED** `3f01a88` (2026-09-16): `exp=<unix seconds>` is an ordinary allow-listed parameter, so it sits inside the signed byte prefix and tampering with it is caught like any scope tamper. REQUIRED on signed links (the signature is the standing grant that gets replayed), optional-but-honoured on unsigned ones (they carry no authority, so demanding a deadline would reject honest links for no gain). Distinct `ExpiredLink`/`MissingExpiry`/`MalformedExpiry` errors; clock injected so expiry is deterministically testable; `sign_link` stamps it before signing. +7 tests, all lanes green on run 35074536797 |
...
| G19 | Low | CI | `release.yml` checkouts at `:25`, `:383`, `:635`, `:750` lack `persist-credentials: false` (the checker pins it only for `audit.yml`/`docs.yml`); the Windows lane runs neither doc tests nor `cargo audit`; `check-model-gates.sh`'s ~230 doc pins execute only on the macOS lanes. | `.github/workflows/release.yml`, `tools/release/check-model-gates.sh` | **WORKING-TREE REPAIR 2026-09-19:** read-only preflight/prebuild/publish checkouts disable persisted credentials; cask finalization retains its required push credentials. Windows CI and release lanes gain doctests and the pinned audit. Workflow-shape assertions and negative fixtures updated; self-test passes. Full doc-count policy still requires macOS, and release execution still needs a real tag. |
| G20 | Info | platform_macos | 130 production `unsafe {}` blocks (2026-09-16 recount: `lib.rs` 67, `ax_worker.rs` 19, `settings_window.rs` 23, `tray.rs` 11, `keychain.rs` 3, `login_item.rs` 3, `url_events.rs` 2, `shell_host.rs` 2); 34 `unsafe fn`, 3 with `# Safety`. No `unsafe impl Send/Sync`; CF retain/release balance, `catch_unwind` on both `extern "C"` callbacks, and single global AX messaging timeout all checked sound. | `crates/platform_macos/src/*.rs` | CONFIRMED, **CLOSED 2026-09-17** under the §21.3 resolution. `a92dc2a` (2026-09-16): `ax_worker.rs` — 17 of its 19 blocks carry a comment naming the precondition each discharges, and the 2 left alone (`RawAxObserverBackend::add_notification`/`remove_notification`) stay listed because their invariant is discharged by a drop-order argument two layers up, so it is annotated at the site that actually discharges it. Annotation commit `6ec61d8` (2026-09-16): `lib.rs` 67/67 (41 adjacent + 26 explicitly-scoped grouped comments — "every `unsafe` call on `element` below" — whose per-function enumerations an independent audit matched against the actual block lists with zero mismatches and zero fabricated claims, checked against the objc2/core-foundation dependency sources) and `shell_host.rs` 2/2; the recount also surfaced four blocks the audit-time count had missed (`tray.rs` 2, `settings_window.rs` 2 — the target-liveness invariant each file's existing annotated sites already record), annotated in the same commit. 128 of 130 blocks annotated; the only unannotated pair is the deliberately listed ax_worker one. Closed on the evidence: local portable gate green incl. `cargo audit` (first run on this host — cargo-audit 0.22.2 installed), and every CI lane green on `6ec61d8`, `fmt · clippy · test (macOS)` included (run 35264496197) |
| G21 | Med | platform_macos | The G6 safety-poll skip introduced by `9b91b35` memoizes on a **single** `last_focused_poll: Option<(String, Option<ScreenRect>)>` slot shared by every `PollFocusedElement`, keyed on neither pid nor notification — but **two** pollers feed it with different kinds (`poll_notification: FocusChanged` and `CaretChanged`) and each consumer hard-filters on kind. `observer_caret_rect` returns `None` for any non-`CaretChanged` notification, so the focus poll always emits `(identity, None)`; whenever the caret rect is *also* unresolvable — non-text focus, Chromium/Electron fields without `AXSelectedTextRange`, or an AX timeout, since both reads are `.ok()?` / `.ok().flatten()` — the caret poll emits the identical pair and is skipped as "unchanged", the dispatch having been spent on a consumer that discarded it. In that steady state every safety poll of **both** kinds after the first is suppressed, defeating the documented A66 250 ms guarantee (`lib.rs:1645-1651`) precisely in the degraded-AX apps the safety net exists for. Conversely in the healthy case the two kinds alternate `(id, None)` / `(id, Some(rect))`, so consecutive polls always differ and **nothing is ever skipped** — the claimed saving is ~zero where AX round trips are expensive. The memo is also never refreshed by the `ObserverEvent` arm, so an X→Y→X round trip inside one 250 ms tick can strand a consumer on Y. | `crates/platform_macos/src/ax_worker.rs:735-775,823,927-933`, `crates/platform_macos/src/lib.rs:1581,1660`, `observer_caret_rect` | CONFIRMED (every link re-verified on the tree 2026-09-16; no live evidence, and no test covers it — the G6 truth-table test drives `focused_poll_changed` in isolation with one caller), **FIXED** `35b6ab8` (2026-09-16, mac lane green on run 35068705271; owner chose repair over removal): the memo is now a `HashMap<(i32, ObserverNotification), (String, Option<ScreenRect>)>` so each poller keeps its own entry, and `remember_dispatched_poll` refreshes it from the `ObserverEvent` arm so the poll tracks what the consumer was actually told rather than only its own history. Three tests: one drives both notification kinds plus a second pid through one memo, one pins the X → Y → X stranding case, and the original truth table moved to the keyed form. Count pins 2158 → 2160 |
...
reviewed by Astra. The repair record below (folded in from the former
`docs/AUDIT-REPAIRS-2026-09-20.md`) describes each regression,
...
in [ROADMAP](docs/ROADMAP.md#full-codebase-audit-repairs--2026-09-20).
