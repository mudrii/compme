...
| G20 | Info | platform_macos | 126 production `unsafe {}` blocks, 80 without a `SAFETY:` comment (`lib.rs` 54/64, `ax_worker.rs` 19/19, `shell_host.rs` 2/2); 34 `unsafe fn`, 3 with `# Safety`. No `unsafe impl Send/Sync`; CF retain/release balance, `catch_unwind` on both `extern "C"` callbacks, and single global AX messaging timeout all checked sound. | `crates/platform_macos/src/*.rs` | CONFIRMED, **PARTIAL** `a92dc2a` (2026-09-16) under the §21.3 resolution: `ax_worker.rs` is done — 17 of its 19 blocks carry a comment naming the precondition each discharges, and the 2 left alone (`RawAxObserverBackend::add_notification`/`remove_notification`) are listed because their invariant is discharged by a drop-order argument two layers up, so it is annotated at the site that actually discharges it. **Remaining: `lib.rs` (54/64) and `shell_host.rs` (2/2)**, same rule — annotate only what is provable, list the rest |
