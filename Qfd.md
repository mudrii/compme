...
| G19 | Low | CI | `release.yml` checkouts at `:25`, `:383`, `:635`, `:750` lack `persist-credentials: false` (the checker pins it only for `audit.yml`/`docs.yml`); the Windows lane runs neither doc tests nor `cargo audit`; `check-model-gates.sh`'s ~230 doc pins execute only on the macOS lanes. | `.github/workflows/release.yml`, `tools/release/check-model-gates.sh:320,337,381` | CONFIRMED, **DEFERRED**: the release workflow checkout shape is exact-pinned by the checker and only a real tag proves a change; revisit at the next release with the Windows doc-test step |
| G20 | Info | platform_macos | 130 production `unsafe {}` blocks (2026-09-16 recount: `lib.rs` 67, `ax_worker.rs` 19, `settings_window.rs` 23, `tray.rs` 11, `keychain.rs` 3, `login_item.rs` 3, `url_events.rs` 2, `shell_host.rs` 2); 34 `unsafe fn`, 3 with `# Safety`. No `unsafe impl Send/Sync`; CF retain/release balance, `catch_unwind` on both `extern "C"` callbacks, and single global AX messaging timeout all checked sound. | `crates/platform_macos/src/*.rs` | CONFIRMED, **ANNOTATIONS COMPLETE — mac-lane CI pending** under the §21.3 resolution. `a92dc2a` (2026-09-16): `ax_worker.rs` — 17 of its 19 blocks carry a comment naming the precondition each discharges, and the 2 left alone (`RawAxObserverBackend::add_notification`/`remove_notification`) stay listed because their invariant is discharged by a drop-order argument two layers up, so it is annotated at the site that actually discharges it. The 2026-09-16 annotation commit: `lib.rs` 67/67 (41 adjacent + 26 explicitly-scoped grouped comments — "every `unsafe` call on `element` below" — whose per-function enumerations an independent audit matched against the actual block lists with zero mismatches and zero fabricated claims, checked against the objc2/core-foundation dependency sources) and `shell_host.rs` 2/2; the recount also surfaced four blocks the audit-time count had missed (`tray.rs` 2, `settings_window.rs` 2 — the target-liveness invariant each file's existing annotated sites already record), annotated in the same commit. 128 of 130 blocks annotated; the only unannotated pair is the deliberately listed ax_worker one. Local portable gate green incl. `cargo audit` (first run on this host — cargo-audit 0.22.2 installed); the mac lane compiles the annotated code and must go green before this row flips to CLOSED |
